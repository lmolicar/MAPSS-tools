...
parser = argparse.ArgumentParser(prog = "generate_mapss_soilsfao.py",
                                 description = "Generate soils (FAO) file for MAPSS")
...
buffer = np.zeros((num_values, 10), dtype = output_type)
...
output_da = xr.DataArray(buffer,
                         dims = ["grid", "band"],
                        attrs = {
                            "long_name" : f"soils_fao",
                            "units" : f"{units}",
                            "scaled" : f"{scale}",
                            "source" : "National soils",
                            "record_length" : "1yr"
                        })
